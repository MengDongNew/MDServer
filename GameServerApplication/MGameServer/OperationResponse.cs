...
{
    // 摘要: 
    //     Incoming Photon.SocketServer.OperationRequest are often answered with a response
    //     (represented by this class).  It can be sent to the client by using the Photon.SocketServer.PeerBase.SendOperationResponse(Photon.SocketServer.OperationResponse,Photon.SocketServer.SendParameters)
    //     method.  The Photon.SocketServer.IRpcProtocol.SerializeOperationResponse(Photon.SocketServer.OperationResponse,Photon.SocketServer.Rpc.Protocols.CustomTypeCache)
    //     method serializes OperationResponse instances.  Photon.SocketServer.OperationResponse.SetParameters(System.Object)
    //     converts properties that are flagged with Photon.SocketServer.Rpc.DataMemberAttribute
    //     into the Photon.SocketServer.OperationResponse.Parameters dictionary.
